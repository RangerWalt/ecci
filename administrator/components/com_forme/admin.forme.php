...
function fieldsCopy($option, $cid){
	...
	if(!empty($cid)){
		...
		$mainframe->redirect('index2.php?option='.$option.'&task=editform&hidemainmenu=1&cid='.$copy_form_id,$msg);
	}else{
		$msg = _FORME_BACKEND_FIELDSCOPY_NONE;
		$mainframe->redirect('index2.php?option='.$option.'&task=editform&hidemainmenu=1&cid='.$form_id,$msg);
	}
}
...
function formsCopy($option, $cid){
	...
	$mainframe->redirect('index2.php?option='.$option.'&task=forms',$msg);
}
...
function fieldsCopyCancel($option){
	...
	$mainframe->redirect('index2.php?option='.$option.'&task=editform&hidemainmenu=1&cid='.$form_id);
}
...
function addSampleData($option){
	...
	$database->query();
	 ...
	 $mainframe->redirect( "index2.php?option=$option", "Sample data added" );
}
...
function cancelform($option){
	...
	$mainframe->redirect( "index2.php?option=$option&task=forms" );
}
...
function cancelfield($option){
	...
	$mainframe->redirect( "index2.php?option=$option".$task.$cid );
}
...
function deletedata($option, $rcid){
	...
	$mainframe->redirect( 'index2.php?option='. $option .'&task=listdata&cid='.$form_id, $msg );
}
...
function deletefield($option, $cid){
	...
	$mainframe->redirect( 'index2.php?option='. $option .'&task=editform&cid='.$form_id, $msg );
}
...
function saveforms($option,$apply=0){
	...
	$row->checkin();
	if(!$apply)	$mainframe->redirect("index2.php?option=$option&task=forms", _FORME_BACKEND_FORMS_SAVE." ");
	else $mainframe->redirect("index2.php?option=$option&task=editform&cid=".$row->id, _FORME_BACKEND_FORMS_SAVE." ");
}
...
function savefield($option,$apply = false){
	...
	$row->default_value = stripslashes($row->default_value);
	$row->params = stripslashes($row->params);
	$row->validation_message = stripslashes($row->validation_message);

	if(empty($row->name)) {
        $mainframe->redirect("index2.php?option=$option&task=editfield&cid=$row->id", _FORME_BACKEND_FIELDS_NAME_EMPTY." "); }
	// store it in the db
	if (!$row -> store()) {
		echo "<script> alert('"
			.$row -> getError()
	...
	if($apply)$mainframe->redirect("index2.php?option=$option&task=editfield&hidemainmenu=1&cid=$row->id", _FORME_BACKEND_FIELDS_SAVE." ");
	else $mainframe->redirect("index2.php?option=$option&task=editform&cid=$row->form_id", _FORME_BACKEND_FIELDS_SAVE." ");
}
...
function orderfield( $id, $inc, $option ) {
	...
	$mainframe->redirect( "index2.php?option=$option&task=editform&cid=".$row->form_id);
}
...
function publishforms( $cid=null, $publishform=1,  $option ) {
  ...
  if (!$database->query()) {
	...
	$mainframe->redirect( 'index2.php?option='.$option.'&task=forms&mosmsg='. $msg );
...
function publishfield( $cid=null, $publishfield=1,  $option ) {
  ...
  if (!$database->query()) {
	...
	$mainframe->redirect( 'index2.php?option='.$option.'&task=editform&cid='.$form_id.'&mosmsg='. $msg );
...
function saveOrder( &$cid ) {
	...
	$mainframe->redirect( 'index2.php?option=com_forme&task=editform&cid='. $row->form_id , $msg );
...
function restoreProcess($option){
	...
	$mainframe->redirect('index2.php?option=com_forme',sprintf(_FORME_BACKEND_RESTORE_MSG,(int)$i));
