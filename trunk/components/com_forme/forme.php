...
defined( '_JEXEC' ) or die( 'Restricted access' );
	...
	function processForm($fid, $processform){
		...
		if(!empty($processform)){
			$errors = false;
			$form_data = $processform;//JRequest::getVar('form',array(),'POST');
			...
			if(strlen($form->thankyou)!=0){
				...
				$mainframe->redirect( str_replace( '&amp;', '&', sefRelToAbs( "index.php?option=com_forme&Itemid=$Itemid&func=thankyou&did=" . md5( $data_id.$row->date_added ))));
			}else {
				if(isset($_SESSION['formdata'])){
					unset($_SESSION['formdata']);
				}
				...
				if($form->return_url!=''){
					...
					$mainframe->redirect(str_replace( '&amp;', '&', sefRelToAbs("index.php?option=com_forme&Itemid=$Itemid&fid=$fid")), _FORME_FRONTEND_REGISTRA_SUCCESS." ");
				}
			}
		}
